...
4. 생성 후 토큰 즉시 복사
5. 토큰 계정은 대상 Space에 **페이지 추가/편집(및 rebuild 시 삭제) 권한**이 있어야 함
...
하위 폴더의 `README.md`가 그 폴더의 **대표 페이지**가 되고, 같은 폴더의 다른 문서는 그 자식으로 생성됩니다. 로컬 `docs/` 구조가 Confluence 페이지 트리로 재현됩니다.

```
[루트 부모 = CONFLUENCE_PARENT_PAGE_ID]
├── docs/*.md (README·WORKLOG·glossary·기획서들)   ← 루트 직속
├── PoC 개발 설계서 (design/README.md)
│   └── item-04/06/08-design                       ← design/README 자식
└── PoC 개발 설계 전 정리 (pre-design/README.md)
    └── item-04/06/08-pre-design                    ← pre-design/README 자식
```

부모는 자식보다 먼저 생성되도록 자동 정렬됩니다. `CONFLUENCE_PARENT_PAGE_ID`를 비우면 루트 직속 문서가 공간 최상위에 생성됩니다.
