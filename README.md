...
**수동 작성**: 아래 표를 보고 직접 `.env` 를 작성해도 됩니다(로컬 개발 시 `cp .env.example .env`).
   ...
   | `CONFLUENCE_PARENT_ID` | (선택) 루트 앵커 — 자식으로 만들 부모 **페이지 또는 폴더** ID. 비우면 공간 최상위. (구 이름 `CONFLUENCE_PARENT_PAGE_ID` 도 인식) |
...
3. **스코프 선택** — 쓰려는 기능에 맞춰 고릅니다. 기본 동기화만 하면 첫 줄 3개로 충분하고, 나머지는 해당 기능을 쓸 때만 추가하세요. (이 표가 스코프 단일 정리본입니다.)

   | 기능 | 필요 스코프 |
   | --- | --- |
   | **기본 동기화** (조회·생성·갱신) | `read:space:confluence` · `read:page:confluence` · `write:page:confluence` |
...
confluence-sync --exclude '*-draft.md' # 동기화 제외(반복 가능). .confluence-syncignore 도 사용
```
...
부모는 자식보다 먼저 생성되도록 자동 정렬됩니다. 폴더 대표는 파일명이 `README.md`로 끝나는 문서(prefix 무관)로 인식합니다. `CONFLUENCE_PARENT_ID`를 비우면 루트 직속 문서가 공간 최상위에 생성됩니다.

### --rebuild (구조를 갈아엎을 때)

부모 관계는 **생성 시점에만** 지정되고 v2 API로는 사후 이동이 어렵습니다. 그래서 계층을 바꾸려면 `--rebuild`로 **매핑된 페이지를 모두 삭제(휴지통) 후 재생성**합니다.

- 삭제 스코프가 필요합니다 → [스코프 표](#api-토큰-발급-scoped-필수) 참고(`delete:page`, 폴더 있으면 `delete:folder`).
- 매핑 파일(`.confluence-sync.json`)에 기록된 페이지·폴더만 삭제하므로, 수동으로 만든 것은 건드리지 않습니다.

## 동기화 제외 (로컬 전용 유지)

같은 폴더에 **Confluence와 동기화할 문서**와 **로컬에만 둘 문서**가 섞여 있을 때, 특정 경로를 sync에서 제외할 수 있습니다.

- **`<base>/.confluence-syncignore`** — `.gitignore` 방식의 패턴 파일(지속적). 문서셋과 함께 커밋하세요.
  ```gitignore
  # 예시
  *-draft.md            # 모든 초안 제외
  notes/local-only.md   # 특정 파일만 제외 (notes/ 의 나머지는 동기화)
  secret/               # 폴더 통째로 제외
  ```
- **`--exclude <glob>`** — 일회성 제외(반복 가능). 예) `confluence-sync --exclude '*-draft.md' --exclude scratch/`

제외된 문서는 **모든 모드**(list·dry-run·sync)에서 빠지고, 그 문서만 들어 있던 폴더는 생성되지 않습니다. 패턴/매칭은 `.gitignore` 규칙(`ignore` 라이브러리)을 그대로 따릅니다(부정 `!`, `**` 등 지원).
...
- 폴더에 `README.md`가 있으면 그 폴더의 대표 **페이지**가 되고, 없으면 Confluence **폴더**가 생성돼 그 밑으로 문서가 동기화됩니다(`write:folder` 스코프 필요).
